...
export default class Debug {
   ...
   */
  public static draw(context: Expo2DContext, level: number = 0) {
    const instance = Debug.getInstance();
    ...
    instance.values.forEach(value => {
      ...
      switch (value.align) {
        case 'left':
          ...
          break;
        case 'right':
          position = vec2(Game.screen.x - instance.options.margin, rightY);
  ...
  private drawLabel(
    ...
    context.fillText(text, x, position.y, Game.screen.x);
    context.restore();
  }
  ...
  private drawChart(
    ...
    if (label) {
      context.fillStyle = foregroundColour;
      context.fillText(label, x, position.y, Game.screen.x);
    }
  ...
  private drawMarker(context: Expo2DContext, marker: DebugMarker) {
    context.save();
  ...
  private drawBorder(context: Expo2DContext, border: DebugBorder) {
    context.save();
